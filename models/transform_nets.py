...
def input_transform_net(point_cloud, is_training, bn_decay=None, K=3):
    ...
    with tf.compat.v1.variable_scope('transform_XYZ') as sc:
        ...
        #                          dtype=tf.float32)
        biases = tf.compat.v1.get_variable('biases', [3*K],
                                   initializer=tf.constant_initializer([1, 0, 0, 0, 1, 0, 0, 0, 1]), dtype=tf.float32)

        # biases.assign(tf.constant([1, 0, 0, 0, 1, 0, 0, 0, 1], dtype=tf.float32))
...
def feature_transform_net(inputs, is_training, bn_decay=None, K=64):
    ...
    with tf.compat.v1.variable_scope('transform_feat') as sc:
        weights = tf.compat.v1.get_variable('weights', [256, K*K],
                                  initializer=tf.constant_initializer(0.0),
                                  dtype=tf.float32)
        # biases = tf.Variable(tf.zeros([K*K], dtype=tf.float32), name='biases')
        biases = tf.compat.v1.get_variable('biases', [K*K],
                                 initializer=tf.constant_initializer(np.eye(K).flatten()),
                                 dtype=tf.float32)
        # biases.assign(tf.constant(np.eye(K).flatten(), dtype=tf.float32))
        transform = tf.matmul(net, weights)
        transform = tf.nn.bias_add(transform, biases)
