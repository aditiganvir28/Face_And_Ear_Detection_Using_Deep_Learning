...
def self_attention_layer(inputs, scope):
    with tf.compat.v1.variable_scope(scope):
        batch_size, num_points, _, channels = inputs.shape
        queries = tf_util.conv2d(inputs, channels // 8, [1, 1], scope="queries")
        keys = tf_util.conv2d(inputs, channels // 8, [1, 1], scope="keys")
        values = tf_util.conv2d(inputs, channels, [1, 1], scope="values")

        attention_map = tf.matmul(queries, keys, transpose_b=True)
        attention_map = tf.nn.softmax(attention_map)
        attention_output = tf.matmul(attention_map, values)
        return inputs + attention_output  # Residual connection
...
def get_model(point_cloud, is_training, bn_decay=None):
    ...
    # Apply Self-Attention Layer after initial feature extraction
    net = self_attention_layer(net, scope="self_attention1")
